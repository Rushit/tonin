...
The CLI binary is named `tonin` and ships from the `crates/tonin` package. The package at `crates/tonin` produces `target/debug/tonin`. Helm chart generation is handled by the `tonin-helm` plugin (`crates/tonin-helm`).
...
The workspace was restructured pre-0.1 from ~10 narrow crates to **Shape B: 6 library crates + 1 CLI binary** to reduce publish/version-bump overhead. What used to be `tonin-transport`, `tonin-discovery`, `tonin-mcp`, and `tonin-telemetry` are now modules inside `tonin-sdk`. At 0.5 the `tonin-plugin` bridge crate was extracted from the CLI so plugin authors get a minimal `tonin.toml` API without pulling in the full CLI dep tree. At 0.6 `tonin-helm` moved into the workspace as `crates/tonin-helm/` — it holds all Helm chart generation and replaced the old `tonin k8s` command. See [`docs/02-architecture.md`](docs/02-architecture.md) for the current shape.
...
**Kubernetes / Helm deploy is handled by `tonin-helm`** (the plugin in `crates/tonin-helm/`). `tonin k8s` is removed; running it prints a migration hint to `tonin helm`.
...
### tonin-helm (`crates/tonin-helm/`)

Helm chart generation plugin. Reads `tonin.toml` via `tonin-plugin` and renders a complete Helm chart into `chart/`. Invoked as `tonin helm generate / diff / upgrade`. Has its own independent version (`crates/tonin-helm/VERSION`) and releases under the `tonin-helm-v*.*.*` tag namespace.
...
2. **Which layer owns it?** A new capability → a trait in `tonin-sdk` (+ a separate impl crate),
   ...
   `crates/tonin/src/commands/` + a `TopCmd` variant (or a `tonin-helm` subcommand for deploy). New
   Helm output → a template in `crates/tonin-helm/` + a plan decision + a test.
...
- Touched a capability or `tonin.toml` field? Update its `docs/NN-*.md` + Status block in the same
  change. Touched a template in `crates/tonin-helm/`? Run **`make gen-example`** (re-renders the
  greeter Helm chart and confirms it builds).
